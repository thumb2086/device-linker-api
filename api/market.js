...
function accountKey(address) {
    return `market:${String(address || "").toLowerCase()}`;
}

function legacyAccountKey(address) {
...
async function hydrateAccountState(userAddress, nowTs, market) {
    ...
    const legacyKey = legacyAccountKey(userAddress);
    const decimals = await settlementService.getDecimals();
    const [walletBalanceWei, totalBet, rawAccount, legacyAccount] = await Promise.all([
    ...
    let account = normalizeMarketAccount(rawAccount || legacyAccount, nowTs);
...
async function buildSnapshotPayload(userAddress) {
    ...
    if (state.legacyKey !== state.key) {
        await kv.del(state.legacyKey);
    }
...
export default async function handler(req, res) {
    ...
    try {
        ...
        try {
            ...
            } else if (action === "trade_stock_batch") {
                actionResult = tradeStockBatch(state.account, market, body.orders);
                const totalSpent = Math.max(0, Number(actionResult.totalSpent || 0));
                const totalReceived = Math.max(0, Number(actionResult.totalReceived || 0));
                await settlementService.settle({
                    userAddress,
                    betWei: ethers.parseUnits(totalSpent.toFixed(6), state.decimals),
                    payoutWei: ethers.parseUnits(totalReceived.toFixed(6), state.decimals),
                    source: MARKET_TX_SOURCE,
                    meta: { action, count: Number(actionResult.count || 0) }
                });
