...
async function buildOverrides(provider, nonce, attempt = 0) {
    ...
    // 關鍵修正：確保 maxFeePerGas >= maxPriorityFeePerGas
    // 否則會觸發 "max priority fee per gas higher than max fee per gas" 錯誤
    if (maxFeePerGas !== undefined && maxFeePerGas < maxPriorityFeePerGas) {
        maxFeePerGas = maxPriorityFeePerGas + (feeData.lastBaseFeePerGas || maxPriorityFeePerGas);
    }
