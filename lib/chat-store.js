...
import { canAccessYjcChatRoom, getYjcChatRoomById, getYjcChatRoomOptions, resolveYjcVipStatus } from "./yjc-vip.js";
...
export async function resolveChatRoomAccess(room, sessionId) {
    ...
    const yjcVip = await resolveYjcVipStatus(session.address);
    const access = canAccessYjcChatRoom(yjcVip, room.id);
    return { ...access, session, yjcVip };
}
...
export function resolveChatRoom(roomId) {
    return getYjcChatRoomById(roomId);
}
