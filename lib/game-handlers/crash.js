...
async function settleCrashLoss({ contract, lossPoolAddress, address, gameState, betId, decimals }) {
    const betWei = ethers.parseUnits(gameState.amount.toString(), decimals);
    ...
    if (buffAdjusted.payoutWei > 0n) {
        const tx = await withQueuedChainTxLock(
            () => transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, address, buffAdjusted.payoutWei, { gasLimit: 200000, txSource: TX_SOURCE }),
            undefined,
            TX_SOURCE
        );
        txHash = tx.hash;
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'start') {
            ...
            const tx = await withQueuedChainTxLock(
                () => sendManagedContractTx(contract, "adminTransfer", [address, lossPoolAddress, betWei], { gasLimit: 200000, txSource: TX_SOURCE }),
                undefined,
                TX_SOURCE
            );
            ...
            const lossPoolAddress = process.env.LOSS_POOL_ADDRESS || wallet.address;
            const tx = await withQueuedChainTxLock(
                () => transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, address, payoutWei, { gasLimit: 200000, txSource: TX_SOURCE }),
                undefined,
                TX_SOURCE
            );
            if (buffAdjusted.changed) {
