...
async function settleCrashLoss({ contract, lossPoolAddress, address, gameState, betId, decimals }) {
    ...
    if (buffAdjusted.payoutWei > 0n) {
        const tx = await withChainTxLock(() => transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, address, buffAdjusted.payoutWei, { gasLimit: 200000 }));
        txHash = tx.hash;
    }
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (action === 'start') {
            ...
            const tx = await withChainTxLock(() => sendManagedContractTx(contract, "adminTransfer", [address, lossPoolAddress, betWei], { gasLimit: 200000 }));
            ...
            const lossPoolAddress = process.env.LOSS_POOL_ADDRESS || wallet.address;
            const tx = await withChainTxLock(() => transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, address, payoutWei, { gasLimit: 200000 }));
            if (buffAdjusted.changed) {
                await persistSettlementBuffProfile(buffAdjusted.profile);
            }
