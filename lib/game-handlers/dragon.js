...
async function compensateDragonBet(contract, lossPoolAddress, sessionAddress, refundWei, txOptions) {
    try {
        await contract.adminTransfer(lossPoolAddress, sessionAddress, refundWei, txOptions);
        return null;
...
async function settleDragonChainRound({
    ...
    try {
        tx = await withChainTxLock(async () => {
            const firstTx = await contract.adminTransfer(sessionAddress, lossPoolAddress, betWei, { gasLimit });
            debitedWei += betWei;
            let lastTx = firstTx;
            if (resultType === "pillar") {
                lastTx = await contract.adminTransfer(sessionAddress, lossPoolAddress, betWei, { gasLimit });
                debitedWei += betWei;
            }
            if (payoutWei > 0n) {
                return transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, sessionAddress, payoutWei, { gasLimit });
            }
            return lastTx;
        });
    } catch (blockchainError) {
        const compensationError = debitedWei > 0n
            ? await compensateDragonBet(contract, lossPoolAddress, sessionAddress, debitedWei, { gasLimit })
            : null;
