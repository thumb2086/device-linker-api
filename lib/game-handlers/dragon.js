...
async function compensateDragonBet(contract, lossPoolAddress, sessionAddress, betWei, txOptions) {
    try {
        await contract.adminTransfer(lossPoolAddress, sessionAddress, betWei, txOptions);
        return null;
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (playMode === "classic" && playAction === "shoot") {
            ...
            try {
                betTx = await contract.adminTransfer(sessionAddress, lossPoolAddress, betWei, { gasLimit: 220000 });
                betTransferred = true;
                if (resultType === "win") {
                    const payoutBigInt = BigInt(Math.floor(multiplier * 100));
                    payoutWei = (betWei * payoutBigInt) / 100n;
                    netWei = payoutWei - betWei;
                    tx = await transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, sessionAddress, payoutWei, { gasLimit: 220000 });
                ...
                const compensationError = betTransferred
                    ? await compensateDragonBet(contract, lossPoolAddress, sessionAddress, betWei, { gasLimit: 220000 })
                    : null;
        ...
        try {
            ...
            const compensationError = betTransferred
                ? await compensateDragonBet(contract, lossPoolAddress, sessionAddress, betWei, { gasLimit: 200000 })
                : null;
            return res.status(500).json({
                error: "區塊鏈交易失敗",
                details: compensationError
