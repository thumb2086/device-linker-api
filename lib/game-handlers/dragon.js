...
async function settleDragonChainRound({
    ...
    let debitedWei = 0n;
    try {
        tx = await withQueuedChainTxLock(async () => {
            const firstTx = await sendManagedContractTx(contract, "adminTransfer", [sessionAddress, lossPoolAddress, betWei], { gasLimit, txSource: TX_SOURCE });
            debitedWei += betWei;
            let lastTx = firstTx;
            if (resultType === "pillar") {
                lastTx = await sendManagedContractTx(contract, "adminTransfer", [sessionAddress, lossPoolAddress, betWei], { gasLimit, txSource: TX_SOURCE });
                debitedWei += betWei;
            }
            if (payoutWei > 0n) {
                return transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, sessionAddress, payoutWei, { gasLimit, txSource: TX_SOURCE });
            }
            return lastTx;
        }, undefined, TX_SOURCE);
    } catch (blockchainError) {
        const compensationError = debitedWei > 0n
            ? await compensateDragonBet(contract, lossPoolAddress, sessionAddress, debitedWei, { gasLimit, txSource: TX_SOURCE })
            : null;
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (playMode === "classic" && playAction === "shoot") {
            ...
            let netWei = -betWei;
            try {
                const settlement = await settleDragonChainRound({
                    contract,
                    lossPoolAddress,
                    sessionAddress,
                    betWei,
