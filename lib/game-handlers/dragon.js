...
async function settleDragonChainRound({
    ...
    try {
        tx = await withChainTxLock(async () => {
            const firstTx = await sendManagedContractTx(contract, "adminTransfer", [sessionAddress, lossPoolAddress, betWei], { gasLimit });
            debitedWei += betWei;
            let lastTx = firstTx;
            if (resultType === "pillar") {
                lastTx = await sendManagedContractTx(contract, "adminTransfer", [sessionAddress, lossPoolAddress, betWei], { gasLimit });
                debitedWei += betWei;
            }
            if (payoutWei > 0n) {
