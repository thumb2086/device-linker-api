...
async function transferStakeIntoEscrow(contract, treasuryAddress, address, decimals) {
    ...
    const chainLock = await acquireChainTxLock(undefined, TX_SOURCE);
    try {
        const tx = await sendManagedContractTx(contract, "adminTransfer", [address, treasuryAddress, stakeWei], { gasLimit: 220000, txSource: TX_SOURCE });
...
async function refundStakeFromEscrow(contract, treasuryAddress, address, decimals) {
    ...
    const chainLock = await acquireChainTxLock(undefined, TX_SOURCE);
    try {
        const tx = await sendManagedContractTx(contract, "adminTransfer", [treasuryAddress, address, stakeWei], { gasLimit: 220000, txSource: TX_SOURCE });
        return { txHash: tx.hash, stakeWei };
    } finally {
        await releaseChainTxLock(chainLock);
    }
...
async function settleMatchPayout(match, winnerAddress, contract, treasuryAddress, decimals) {
    ...
    const chainLock = await acquireChainTxLock(undefined, TX_SOURCE);
    try {
        const tx = await sendManagedContractTx(contract, "adminTransfer", [treasuryAddress, winnerAddress, payoutWei], { gasLimit: 240000, txSource: TX_SOURCE });
