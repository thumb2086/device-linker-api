    ...
    try {
        ...
        try {
            tx = await withQueuedChainTxLock(async () => {
                betTx = await sendManagedContractTx(contract, "adminTransfer", [address, lossPoolAddress, betWei], { gasLimit: 200000, txSource: TX_SOURCE });
                if (payoutWei > 0n) {
                    return transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, address, payoutWei, { gasLimit: 200000, txSource: TX_SOURCE });
                }
                return betTx;
            }, undefined, TX_SOURCE);
        ...
        await recordGameHistory({
            ...
            outcomeLabel: result.isWin ? "中獎" : "未中",
            betWei,
            payoutWei,
            netWei,
            ...
            txHash: tx.hash,
            details: `${betType}:${betValue} / 開出 ${winningNumber} ${winningColor}`,
            decimals
        });
