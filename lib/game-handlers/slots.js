    ...
    try {
        ...
        try {
            tx = await withChainTxLock(async () => {
                betTx = await sendManagedContractTx(contract, "adminTransfer", [sessionAddress, lossPoolAddress, betWei], { gasLimit: 220000, txSource: TX_SOURCE });
                betTransferred = true;
                if (payoutWei > 0n) {
                    return transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, sessionAddress, payoutWei, { gasLimit: 220000, txSource: TX_SOURCE });
                }
                return betTx;
            }, undefined, TX_SOURCE);
            ...
            const compensationError = betTransferred
                ? await compensateSlotsBet(contract, lossPoolAddress, sessionAddress, betWei, { gasLimit: 220000, txSource: TX_SOURCE })
                : null;
