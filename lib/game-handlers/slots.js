...
async function settleSlotSpin({ spinId, sessionAddress }) {
    ...
    try {
        ...
        try {
            ...
            if (!betTransferred) {
                const betTx = await withQueuedChainTxLock(async () => {
                    return sendManagedContractTx(contract, "adminTransfer", [sessionAddress, lossPoolAddress, betWei], {
                        gasLimit: 120000,
                        txSource: TX_SOURCE,
            ...
            let payoutTxHash = "";
            if (payoutWei > 0n) {
                const payoutTx = await withQueuedChainTxLock(async () => {
