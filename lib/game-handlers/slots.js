...
function activeSpinKey(address) {
    return `slots_active_spin:${String(address || "").trim().toLowerCase()}`;
}
...
async function clearActiveSpinIfMatches(address, spinId) {
    const key = activeSpinKey(address);
    const current = await kv.get(key);
    if (String(current || "") === String(spinId || "")) {
        await kv.del(key);
    }
}

async function loadActiveSpin(address) {
    const activeSpinId = String(await kv.get(activeSpinKey(address)) || "").trim();
    if (!activeSpinId) return null;
    const spin = await kv.get(spinKey(activeSpinId));
    if (!spin || String(spin.address || "").toLowerCase() !== String(address || "").toLowerCase()) {
        await kv.del(activeSpinKey(address));
        return null;
    }
    ...
    try {
        ...
        try {
            tx = await withChainTxLock(async () => {
                ...
                await kv.set(key, spin, { ex: SLOTS_SPIN_TTL_SECONDS });
                if (payoutWei > 0n) {
                    return transferFromTreasuryWithAutoTopup(contract, lossPoolAddress, sessionAddress, payoutWei, {
                        gasLimit: 220000,
                        txSource: TX_SOURCE
                    });
                }
            ...
            if (betTransferred) {
                compensationError = await compensateSlotsBet(contract, lossPoolAddress, sessionAddress, betWei, {
                    gasLimit: 220000,
                    txSource: TX_SOURCE
                });
            }
            ...
            await clearActiveSpinIfMatches(sessionAddress, spinId);
            return failedRecord;
        }
...
async function createPendingSpin({ sessionAddress, amount, currentTotalBet, decimals }) {
    ...
    await kv.set(activeSpinKey(sessionAddress), spinId, { ex: SLOTS_SPIN_TTL_SECONDS });
    return spinRecord;
}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (currentAction === "status") {
            ...
            let spin = targetSpinId ? await kv.get(spinKey(targetSpinId)) : await loadActiveSpin(sessionAddress);
            if (!spin || String(spin.address || "").toLowerCase() !== sessionAddress) {
        ...
        const activeSpin = await loadActiveSpin(sessionAddress);
        if (activeSpin) {
            return res.status(200).json(buildSpinResponse(activeSpin, { resumed: true }));
        }
