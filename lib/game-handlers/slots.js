...
import { checkManagedTxReceipt } from "../admin-chain.js";
import { logApiErrorEvent } from "../tx-monitor.js";

const TX_SOURCE = "slots";
const SLOTS_SPIN_TTL_SECONDS = 900;
const SETTLEMENT_VERSION = 3;

const TRIPLE_HIT_RATE = 0.045;
const MAX_DOUBLE_LINES_ON_NORMAL_SPIN = 1;
const SYMBOLS = [
...
async function spinBoard(address) {
    ...
    while (true) {
        ...
        if (result.tripleCount === 0 && result.doubleCount <= MAX_DOUBLE_LINES_ON_NORMAL_SPIN) {
            return board;
        }
    }
}
...
async function resolveManagedSpinTx(txHash, txContext = {}) {
    if (!txHash) return null;
    return checkManagedTxReceipt(settlementService.provider, { hash: txHash, nonce: txContext.nonce }, txContext);
}

async function logSlotsFailureEvent({ spin = null, spinId = "", sessionAddress = "", stage = "", error = "", requestId = "" }) {
    const resolvedSpinId = String(spinId || (spin && spin.spinId) || "").trim();
    const resolvedStage = String(stage || (spin && spin.settlementStage) || "unknown").trim().toLowerCase() || "unknown";
    const resolvedAddress = String(sessionAddress || (spin && spin.address) || "").trim().toLowerCase();
    const resolvedError = String(error || (spin && spin.settlementError) || (spin && spin.error) || "slots settlement failed").trim();
    const resolvedBetTxHash = String(spin && spin.betTxHash || "").trim();
    const resolvedPayoutTxHash = String(spin && spin.payoutTxHash || "").trim();
    const eventId = resolvedSpinId ? `slots_failure_${resolvedSpinId}_${resolvedStage}` : "";

    await logApiErrorEvent({
        id: eventId || undefined,
        kind: "slots_settlement_failure",
        method: "POST",
        source: "game:slots",
        route: "/api/game",
        ...
        meta: {
            ...
            payoutTxHash: resolvedPayoutTxHash
        }
    }).catch(() => {});
}
...
async function settleSlotSpin({ spinId, sessionAddress }) {
    ...
    try {
        ...
        spin = {
            ...
            buffChanged,
            buffProfile,
            status: needsBetTransfer ? "pending" : (payoutRequired ? "settling" : "pending"),
            payoutStatus: payoutRequired ? (spin.payoutStatus || "pending") : "not_needed",
        ...
        if (needsBetTransfer) {
            if (!spin.betTxHash || spin.betStatus === "failed") {
                try {
                    ...
                    const pendingBetRecord = {
                        ...
                        betTxContext: betResults.betTxContext || null,
                        updatedAt: new Date().toISOString()
                    };
                    await kv.set(key, pendingBetRecord, { ex: SLOTS_SPIN_TTL_SECONDS });
                    return pendingBetRecord;
                } catch (error) {
                    const failedRecord = {
                        ...spin,
                        status: "failed",
            ...
            try {
                const betReceipt = await resolveManagedSpinTx(spin.betTxHash, spin.betTxContext || {
                    methodName: "adminTransfer",
                    txSource: TX_SOURCE,
                    txMeta: { spinId, stage: "bet" }
                });
                if (!betReceipt) return spin;
            } catch (error) {
                const failedRecord = {
                    ...spin,
                    status: "failed",
        ...
        if (!payoutRequired) {
            ...
            const settledRecord = {
                ...spin,
                status: "settled",
                payoutStatus: "not_needed",
                settlementStage: "completed",
                settlementError: "",
                error: "",
                finalTxHash: spin.betTxHash || "",
                payoutAmount: 0,
                payoutWei: payoutWei.toString(),
                netWei: netWei.toString(),
        ...
        if (!spin.payoutTxHash || spin.payoutStatus === "failed") {
            try {
                const payoutResults = await settlementService.settle({
                    userAddress: sessionAddress,
                    betWei: 0n,
                    payoutWei,
                    source: TX_SOURCE,
                ...
                const payoutPendingRecord = {
                    ...spin,
                    status: "settling",
                    payoutStatus: "pending",
                    settlementStage: "payout",
                    payoutTxHash: payoutResults.payoutTxHash || "",
                    payoutTxContext: payoutResults.payoutTxContext || null,
                    settlementError: "",
                    error: "",
                    updatedAt: new Date().toISOString()
                };
                await kv.set(key, payoutPendingRecord, { ex: SLOTS_SPIN_TTL_SECONDS });
                return payoutPendingRecord;
            } catch (error) {
                ...
                await kv.set(key, failedPayoutRecord, { ex: SLOTS_SPIN_TTL_SECONDS });
                await logSlotsFailureEvent({
                    spin: failedPayoutRecord,
                    stage: "payout_broadcast",
                    sessionAddress
                });
                return failedPayoutRecord;
            }
        }

        try {
            const payoutReceipt = await resolveManagedSpinTx(spin.payoutTxHash, spin.payoutTxContext || {
                methodName: "adminTransfer",
                txSource: TX_SOURCE,
                txMeta: { spinId, stage: "payout" }
            });
            if (!payoutReceipt) return spin;
        } catch (error) {
            const failedPayoutRecord = {
                ...spin,
                status: "failed_payout",
        ...
        const settledRecord = {
            ...
            settlementError: "",
            error: "",
            finalTxHash: spin.payoutTxHash || spin.betTxHash || "",
...
async function createSpinAfterBet({ sessionAddress, amount, currentTotalBet, decimals }) {
    ...
    const vipStatus = buildVipStatus(currentTotalBet || 0);
...
export default async function handler(req, res) {
    ...
    try {
        ...
        await logSlotsFailureEvent({
            spinId,
            sessionAddress: String(address || "").trim(),
            stage: currentAction || "handler",
            error: error.message,
            requestId
        });
