...
async function settleSlotSpin({ spinId, sessionAddress }) {
    ...
    try {
        ...
        spin = { ...spin, status: "settling", updatedAt: new Date().toISOString() };
        ...
        try {
            const results = await settlementService.settle({
                ...
                betWei: betTransferred ? 0n : betWei, // Skip if already deducted
                payoutWei,
                source: TX_SOURCE,
                meta: { spinId }
            });

            if (buffAdjusted.changed) await persistSettlementBuffProfile(buffAdjusted.profile);

            const totalBetRaw = await recordTotalBet(sessionAddress, parseFloat(spin.amount));
            ...
            const finalTxHash = results.payoutTxHash || results.betTxHash || spin.betTxHash;
            ...
            const isBetActuallyTransferred = betTransferred || results.betTransferred;
            
            const failedRecord = {
                ...spin,
                status: isBetActuallyTransferred ? "failed_payout" : "failed",
                betTransferred: isBetActuallyTransferred,
                betTxHash: results.betTxHash || spin.betTxHash,
                error: error.message,
