...
export function buildMarketSnapshot(nowTs = Date.now()) {
    ...
    for (let pointIndex = 0; pointIndex < MARKET_HISTORY_POINTS; pointIndex += 1) {
        let totalRatio = 0;
        for (const symbol of symbolKeys) {
            const meta = MARKET_SYMBOLS[symbol];
            const series = history[symbol] || [];
            const price = Number(series[pointIndex] || meta.basePrice);
            totalRatio += meta.basePrice > 0 ? price / meta.basePrice : 1;
        }
        const averageRatio = symbolKeys.length > 0 ? totalRatio / symbolKeys.length : 1;
        marketIndexHistory.push(round(averageRatio * 100, 4));
    }
    ...
    const currentMarketIndex = marketIndexHistory.length > 0 ? marketIndexHistory[marketIndexHistory.length - 1] : 100;
    const previousMarketIndex = marketIndexHistory.length > 1 ? marketIndexHistory[marketIndexHistory.length - 2] : currentMarketIndex;
    const marketTrendPct = previousMarketIndex > 0
        ? round(((currentMarketIndex - previousMarketIndex) / previousMarketIndex) * 100, 4)
        : 0;
    const avgMove = symbolKeys.length > 0 ? moveAccumulator / symbolKeys.length : 0;
    ...
    return {
        ...
        marketIndex: round(currentMarketIndex, 4),
        marketTrendPct,
