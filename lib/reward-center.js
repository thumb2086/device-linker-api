...
const SYSTEM_TITLE_PREFIX = "system_title:";
...
const SYSTEM_TITLE_RANK_KEYS = {
    weekly_champion: { key: `${SYSTEM_TITLE_PREFIX}weekly_champion`, expiresDays: 7 },
    monthly_champion: { key: `${SYSTEM_TITLE_PREFIX}monthly_champion`, expiresDays: 30 },
    season_king: { key: `${SYSTEM_TITLE_PREFIX}season_king`, expiresDays: 90 }
};
...
async function applySystemTitleAchievements(profile, totalBet = 0) {
    ...
    for (const titleId of rankKeys) {
        ...
        if (winner && winner === address && !hasActiveTitle(titleId)) {
            profile.ownedTitles = grantOwnedEntry(profile.ownedTitles, titleId, "system", daysFromNowIso(config.expiresDays));
            changed = true;
        }
    }
