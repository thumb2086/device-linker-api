...
import { buildVipStatus, meetsVipLevelRequirement } from "./level.js";
...
const AVATAR_CATALOG_PREFIX = "reward_avatar_catalog:";
const DEFAULT_AVATAR_ID = "classic_chip";
const SYSTEM_TITLE_THRESHOLDS = [
    { id: "million_winner", threshold: 1_000_000 },
    { id: "ten_million_winner", threshold: 10_000_000 },
    { id: "hundred_million_winner", threshold: 100_000_000 },
    { id: "hundred_billion_winner", threshold: 100_000_000_000 }
];
const SYSTEM_TITLE_RANK_KEYS = {
    weekly_champion: { type: "weekly", key: "system_title:weekly:current_winner", periodKey: "week" },
    monthly_champion: { type: "monthly", key: "system_title:monthly:current_winner", periodKey: "month" },
    season_king: { type: "season", key: "system_title:season:current_winner", periodKey: "season" }
};

const AVATAR_CATALOG = [
    { id: "classic_chip", name: "經典籌碼", rarity: "common", icon: "🪙", source: "default", description: "子熙賭場最經典的籌碼圖案。" },
    { id: "cue_master", name: "球桌王牌", rarity: "rare", icon: "🎱", source: "shop", description: "專為撞球愛好者設計的王牌頭像。" },
    { id: "neon_dice", name: "霓虹骰魂", rarity: "rare", icon: "🎲", source: "shop", description: "充滿賽博朋克風格的動態骰子。" },
    { id: "gold_dragon", name: "金龍印記", rarity: "epic", icon: "🐉", source: "chest", description: "象徵權威與財富的東方金龍。" },
    { id: "celestial_crown", name: "星冠信標", rarity: "mythic", icon: "👑", source: "campaign", description: "璀璨奪目的星之冠冕。" },
    { id: "admin_shield", name: "管理聖盾", rarity: "legendary", icon: "🛡️", source: "admin", description: "官方管理人員專屬的神聖護盾。" }
];

const BASE_TITLE_CATALOG = [
    { id: "yuanlao_figure", name: "元老人物", rarity: "mythic", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "子熙賭場的早期元老，見證了最初的發展。" },
    { id: "founding_player", name: "開服玩家", rarity: "epic", source: "campaign", adminGrantable: true, showOnLeaderboard: true, description: "在開服初期便加入的資深冒險者。" },
    { id: "closed_beta_member", name: "封測成員", rarity: "epic", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "參與過封閉測試的尊貴成員。" },
    { id: "beta_witness", name: "內測見證者", rarity: "epic", source: "campaign", adminGrantable: true, showOnLeaderboard: true, description: "內測時期的歷史見證人。" },
    { id: "legendary_player", name: "傳奇玩家", rarity: "legendary", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "在賭場留下了無數傳說的頂尖玩家。" },
    { id: "million_winner", name: "百萬贏家", rarity: "rare", source: "system", adminGrantable: true, showOnLeaderboard: true, description: "累積贏得超過一百萬子熙幣的贏家。" },
    { id: "ten_million_winner", name: "千萬贏家", rarity: "epic", source: "system", adminGrantable: true, showOnLeaderboard: true, description: "累積贏得超過一千萬子熙幣的頂級贏家。" },
    { id: "hundred_million_winner", name: "億級贏家", rarity: "mythic", source: "system", adminGrantable: true, showOnLeaderboard: true, description: "累積贏得超過一億子熙幣的傳奇大亨。" },
    { id: "hundred_billion_winner", name: "百億贏家", rarity: "legendary", source: "system", adminGrantable: true, showOnLeaderboard: true, description: "站在財富頂點的百億級神話。" },
    { id: "weekly_champion", name: "周榜冠軍", rarity: "rare", source: "system", adminGrantable: true, showOnLeaderboard: true, description: "曾在週榜競爭中脫穎而出的冠軍。" },
    { id: "monthly_champion", name: "月榜冠軍", rarity: "epic", source: "system", adminGrantable: true, showOnLeaderboard: true, description: "在月度排行榜中傲視群雄的王者。" },
    { id: "season_king", name: "賽季王者", rarity: "mythic", source: "system", adminGrantable: true, showOnLeaderboard: true, description: "統治了整個賽季的最終王者。" },
    { id: "event_champion", name: "活動冠軍", rarity: "rare", source: "campaign", adminGrantable: true, showOnLeaderboard: true, description: "在限時活動中奪得魁首的佼佼者。" },
    { id: "event_legend", name: "活動傳奇", rarity: "legendary", source: "campaign", adminGrantable: true, showOnLeaderboard: true, description: "在活動歷史中留下不朽印記的傳奇。" },
    { id: "official_certified", name: "官方認證", rarity: "epic", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "經過官方正式認證的信譽玩家。" },
    { id: "official_guest", name: "官方特邀", rarity: "epic", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "受邀參加官方特別活動的貴賓。" },
    { id: "zixi_hot_burn", name: "子熙好燒", rarity: "legendary", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "子熙親自認證：這個人好燒啊！" },
    { id: "admin_operator", name: "管理員", rarity: "legendary", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "維護賭場秩序的幕後管理者。" },
    { id: "genesis_supporter", name: "創世支持者", rarity: "mythic", source: "campaign", adminGrantable: true, showOnLeaderboard: true, description: "最初提供寶貴支持的創世支持者。" },
    { id: "casino_sage", name: "賭場智者", rarity: "epic", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "在賭場策略與心態上令人敬佩的智者。" },
    { id: "lucky_star", name: "幸運之星", rarity: "rare", source: "campaign", adminGrantable: true, showOnLeaderboard: true, description: "在活動中展現超高幸運值的玩家。" },
    { id: "streak_master", name: "連勝之王", rarity: "mythic", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "保持長時間連勝紀錄的頂尖高手。" },
    { id: "market_maestro", name: "市場操盤手", rarity: "epic", source: "campaign", adminGrantable: true, showOnLeaderboard: true, description: "在市場模擬中展現卓越操作的操盤手。" },
    { id: "battle_hand", name: "戰神之手", rarity: "legendary", source: "admin", adminGrantable: true, showOnLeaderboard: true, description: "在關鍵戰局中掌控勝負的傳奇之手。" }
];
...
const SHOP_ITEM_CATALOG = [
    {
        ...
        type: "buff",
        price: 8000000,
        description: "15 分鐘內淨盈利 x2。",
        effect: { effectType: "profit_boost", durationMinutes: 15, multiplier: 2, maxBonus: 0, stackable: false, eligibleScopes: ["solo"] }
    },
    {
        ...
        type: "buff",
        price: 300000000,
        description: "30 分鐘內淨盈利 x2。",
        effect: { effectType: "profit_boost", durationMinutes: 30, multiplier: 2, maxBonus: 0, stackable: false, eligibleScopes: ["solo"] }
    },
    {
...
const KEY_PACK_RULES = {
    basic_key: {
        rewards: [
            { weight: 35, rewards: { tokens: 120000 }, rarity: "common" },
            { weight: 20, rewards: { items: [{ id: "profit_boost_small", qty: 1 }] }, rarity: "rare" },
            { weight: 15, rewards: { items: [{ id: "loss_shield_single", qty: 1 }] }, rarity: "epic" },
            { weight: 12, rewards: { items: [{ id: "rare_chest", qty: 1 }] }, rarity: "epic" },
            { weight: 8, rewards: { tokens: 250000, items: [{ id: "basic_key", qty: 1 }] }, rarity: "rare" },
            { weight: 6, rewards: { avatars: ["cue_master"] }, rarity: "mythic" },
            { weight: 4, rewards: { titles: [{ id: "million_winner" }] }, rarity: "mythic" }
        ]
    },
    advanced_key: {
        rewards: [
            { weight: 25, rewards: { tokens: 600000 }, rarity: "rare" },
            { weight: 20, rewards: { items: [{ id: "profit_boost_small", qty: 2 }] }, rarity: "epic" },
            { weight: 15, rewards: { items: [{ id: "loss_shield_single", qty: 2 }] }, rarity: "epic" },
            { weight: 12, rewards: { items: [{ id: "epic_chest", qty: 1 }] }, rarity: "mythic" },
            { weight: 10, rewards: { tokens: 1500000, items: [{ id: "advanced_key", qty: 1 }] }, rarity: "epic" },
            { weight: 8, rewards: { avatars: ["neon_dice"] }, rarity: "mythic" },
            { weight: 6, rewards: { titles: [{ id: "weekly_champion" }] }, rarity: "legendary" },
            { weight: 4, rewards: { titles: [{ id: "ten_million_winner" }] }, rarity: "legendary" }
        ]
    },
    master_key: {
        rewards: [
            { weight: 20, rewards: { tokens: 8000000 }, rarity: "epic" },
            { weight: 18, rewards: { items: [{ id: "profit_boost_large", qty: 1 }] }, rarity: "mythic" },
            { weight: 15, rewards: { items: [{ id: "loss_shield_timed", qty: 1 }] }, rarity: "mythic" },
            { weight: 12, rewards: { items: [{ id: "legendary_chest", qty: 1 }] }, rarity: "legendary" },
            { weight: 10, rewards: { tokens: 20000000, items: [{ id: "master_key", qty: 1 }] }, rarity: "legendary" },
            { weight: 10, rewards: { avatars: ["celestial_crown"] }, rarity: "mythic" },
            { weight: 8, rewards: { titles: [{ id: "official_certified" }] }, rarity: "legendary" },
            { weight: 7, rewards: { titles: [{ id: "hundred_million_winner" }] }, rarity: "legendary" }
        ]
    }
};

const TITLE_DUPLICATE_COMPENSATION = {
    common: 500000,
    rare: 2000000,
    super_rare: 5000000,
    epic: 8000000,
...
const AVATAR_DUPLICATE_COMPENSATION = {
    common: 200000,
    rare: 1500000,
    super_rare: 4000000,
    epic: 6000000,
...
let cachedAvatarCatalogAt = 0;
let cachedAvatarCatalog = null;
...
function normalizeTitleCatalogEntry(record = {}) {
    ...
    return {
        ...
        description: trimText(record.description, 240),
...
function getDuplicateTitleCompensation(title) {
    const rarity = String(title && title.rarity || "").toLowerCase();
    return asNonNegativeInt(TITLE_DUPLICATE_COMPENSATION[rarity], 1000000);
}

function getDuplicateAvatarCompensation(avatar) {
    const rarity = String(avatar && avatar.rarity || "").toLowerCase();
    return asNonNegativeInt(AVATAR_DUPLICATE_COMPENSATION[rarity], 500000);
}
...
export async function getAvatarCatalog(forceRefresh = false) {
    if (!forceRefresh && cachedAvatarCatalog && Date.now() - cachedAvatarCatalogAt < 30 * 1000) {
        return clone(cachedAvatarCatalog);
    }
    const dynamicAvatars = [];
    for await (const key of kv.scanIterator({ match: `${AVATAR_CATALOG_PREFIX}*`, count: 1000 })) {
        const record = await kv.get(key);
        if (record && record.id) dynamicAvatars.push(record);
    }
    const merged = clone(AVATAR_CATALOG);
    dynamicAvatars.forEach((item) => {
    ...
    merged.sort((left, right) => String(left.name || "").localeCompare(String(right.name || ""), "zh-Hant"));
    cachedAvatarCatalog = merged;
    cachedAvatarCatalogAt = Date.now();
    return clone(cachedAvatarCatalog);
}

async function getAvatarCatalogMap(forceRefresh = false) {
    return rewardCatalogIndex(await getAvatarCatalog(forceRefresh));
}

export async function upsertRewardAvatar(input) {
    const requestedId = trimText(input && input.id, 64) || toSlug(input && input.name);
    if (!requestedId) throw new Error("Missing avatar ID");
    const existingInBase = AVATAR_CATALOG.find(a => a.id === requestedId);
    const storedRecord = await kv.get(`${AVATAR_CATALOG_PREFIX}${requestedId}`);
    const existing = storedRecord || existingInBase || {};
    const normalized = {
        id: requestedId,
        name: trimText(input.name || existing.name, 80),
        rarity: trimText(input.rarity || existing.rarity, 24) || "common",
        icon: trimText(input.icon || existing.icon, 16) || "👤",
        source: trimText(input.source || existing.source, 32) || "admin",
        description: trimText(input.description || existing.description, 240),
        updatedAt: nowIso()
    };
    if (!normalized.name) throw new Error("Avatar name is required");
    await kv.set(`${AVATAR_CATALOG_PREFIX}${normalized.id}`, normalized);
    cachedAvatarCatalog = null;
    cachedAvatarCatalogAt = 0;
    return normalized;
}
...
function normalizeProfile(record, address) {
    ...
    const normalized = {
        ...
        systemTitleStreaks: (base && typeof base.systemTitleStreaks === "object") ? base.systemTitleStreaks : {},
...
function daysFromNowIso(days) {
    ...
    return new Date(Date.now() + numericDays * 24 * 60 * 60 * 1000).toISOString();
}

async function applySystemTitleAchievements(profile, totalBet = 0) {
    let changed = false;
    const normalizedBet = Number(totalBet || 0);
    const activeTitles = removeExpiredEntries(profile.ownedTitles);
    const hasActiveTitle = (titleId) => activeTitles.some((entry) => entry.id === titleId && isEntryActive(entry));
    const periodSnapshot = getPeriodSnapshot();

    SYSTEM_TITLE_THRESHOLDS.forEach((rule) => {
        if (normalizedBet >= rule.threshold && !hasActiveTitle(rule.id)) {
            profile.ownedTitles = grantOwnedEntry(profile.ownedTitles, rule.id, "system", "");
            changed = true;
        }
    });

    const address = String(profile.address || "").trim().toLowerCase();
    const rankKeys = Object.keys(SYSTEM_TITLE_RANK_KEYS);
    for (const titleId of rankKeys) {
        const config = SYSTEM_TITLE_RANK_KEYS[titleId];
        const winner = String(await kv.get(config.key) || "").trim().toLowerCase();
        const period = periodSnapshot[config.periodKey];
        const expiresAt = period ? new Date(period.endMs).toISOString() : "";
        if (winner && winner === address && !hasActiveTitle(titleId)) {
            profile.ownedTitles = grantOwnedEntry(profile.ownedTitles, titleId, "system", expiresAt);
            changed = true;
        }
    }
...
export async function getRewardCatalog() {
    return {
        ...
        rules: {
            chests: clone(CHEST_RULES),
            keys: clone(KEY_PACK_RULES)
        }
    };
}
...
export function buildVipRewardTitle(totalBet) {
    ...
    return {
        ...
        rarity: vipStatus.vipLevel.indexOf("等級") >= 0 ? "epic" : "rare",
        source: "vip",
        showOnLeaderboard: true,
        isVirtual: true,
        description: `當前等級為 ${vipStatus.vipLevel}，享有對應的下注上限與專屬身份標識。`
    };
}
...
async function resolveEquippedTitle(profile, totalBet) {
    ...
    if (profile.selectedTitleId) {
        ...
        if (catalogTitle) {
            return {
                id: catalogTitle.id,
                name: applySystemTitleStreakLabel(catalogTitle.name, catalogTitle.id, profile),
                rarity: catalogTitle.rarity,
                source: owned.source || catalogTitle.source,
                showOnLeaderboard: !!catalogTitle.showOnLeaderboard,
                description: catalogTitle.description || catalogTitle.shopDescription || "",
...
async function resolveAvatar(profile) {
    ...
    const avatarMap = await getAvatarCatalogMap();
...
function applySystemTitleStreakLabel(name, titleId, profile) {
    const streaks = profile && typeof profile.systemTitleStreaks === "object" ? profile.systemTitleStreaks : {};
    let count = 0;
    if (titleId === "weekly_champion") count = Number(streaks.weekly && streaks.weekly.count || 0);
    if (titleId === "monthly_champion") count = Number(streaks.monthly && streaks.monthly.count || 0);
    if (titleId === "season_king") count = Number(streaks.season && streaks.season.count || 0);
    if (!Number.isFinite(count) || count <= 0) return name;

    if (titleId === "weekly_champion") return `${name}（連續${count}週榜一）`;
    if (titleId === "monthly_champion") return `${name}（連續${count}月榜一）`;
    if (titleId === "season_king") return `${name}（連續${count}季榜一）`;
    return name;
}
...
export async function buildRewardSummaryFromProfile(profile, totalBet = 0) {
    ...
    return {
        ...
        avatar: await resolveAvatar(profile),
        title: equippedTitle,
        ...
        titles: summarizeOwnedEntries(profile.ownedTitles, titleMap, (catalog, owned) => ({
            id: catalog.id,
            name: applySystemTitleStreakLabel(catalog.name, catalog.id, profile),
            rarity: catalog.rarity,
            source: owned.source || catalog.source,
            showOnLeaderboard: !!catalog.showOnLeaderboard,
            description: catalog.description || catalog.shopDescription || "",
...
export async function buildRewardSummary(address, totalBet = 0) {
    ...
    const changed = await applySystemTitleAchievements(profile, totalBet);
    if (changed) {
...
export async function grantRewardBundle(address, bundle, meta = {}) {
    ...
    const tokenRewards = bundle && typeof bundle === "object" && bundle.tokenRewards && typeof bundle.tokenRewards === "object"
        ? bundle.tokenRewards
        : { zxc: Number(bundle && bundle.tokens || 0) };
    return {
        profile: saved,
        tokens: Number(tokenRewards.zxc || 0),
        tokenRewards
    };
}
...
export async function openChest(address, chestItemId) {
    ...
    const adjustedRewards = clone(selectedReward.rewards || {});
    const rewardNotices = [];
    if (Array.isArray(adjustedRewards.avatars) && adjustedRewards.avatars.length) {
        const avatarMap = await getAvatarCatalogMap();
        const ownedAvatarIds = new Set(removeExpiredEntries(profile.ownedAvatars).map((entry) => entry.id));
        let compensationTokens = 0;
        adjustedRewards.avatars = adjustedRewards.avatars.filter((avatarId) => {
            const avatar = avatarMap.get(avatarId);
            if (!avatarId || !avatar) return false;
            if (ownedAvatarIds.has(avatarId)) {
                const compensation = getDuplicateAvatarCompensation(avatar);
                compensationTokens += compensation;
                rewardNotices.push(`${avatar.name} 重複，改補償 ${compensation.toLocaleString()} 子熙幣`);
                return false;
            }
            ownedAvatarIds.add(avatarId);
            return true;
        });
        if (!adjustedRewards.avatars.length) delete adjustedRewards.avatars;
        if (compensationTokens > 0) {
            adjustedRewards.tokens = Number(adjustedRewards.tokens || 0) + compensationTokens;
        }
    }
    if (Array.isArray(adjustedRewards.titles) && adjustedRewards.titles.length) {
        const titleMap = await getTitleCatalogMap();
        const ownedTitleIds = new Set(removeExpiredEntries(profile.ownedTitles).map((entry) => entry.id));
        let compensationTokens = 0;
        adjustedRewards.titles = adjustedRewards.titles.filter((titleEntry) => {
            const titleId = typeof titleEntry === "string" ? titleEntry : titleEntry && titleEntry.id;
            const title = titleMap.get(titleId);
            if (!titleId || !title) return false;
            if (ownedTitleIds.has(titleId)) {
                const compensation = getDuplicateTitleCompensation(title);
                compensationTokens += compensation;
                rewardNotices.push(`${title.name} 重複，改補償 ${compensation.toLocaleString()} 子熙幣`);
                return false;
            }
            ownedTitleIds.add(titleId);
            return true;
        });
        if (!adjustedRewards.titles.length) delete adjustedRewards.titles;
        if (compensationTokens > 0) {
            adjustedRewards.tokens = Number(adjustedRewards.tokens || 0) + compensationTokens;
        }
    }

    await applyGrantBundleToProfile(profile, adjustedRewards, { source: `chest:${chestItemId}` });
    const saved = await saveRewardProfile(profile);
...
function normalizeCampaignRecord(record) {
    ...
    return {
        ...
        minLevel: trimText(raw.minLevel || raw.minVipLevel, 64),
        minVipLevel: trimText(raw.minLevel || raw.minVipLevel, 64),
...
export async function listRewardCampaigns(options = {}) {
    ...
    if (keys.length === 0) return { total: 0, campaigns: [] };

    const rawRecords = await Promise.all(keys.map(key => kv.get(key)));
    const records = rawRecords
        .map(raw => normalizeCampaignRecord(raw))
        .filter(record => {
    ...
    const nowMs = Date.now();
    async function decorateCampaign(record) {
        if (!address) {
            return {
                ...record,
                claimCount: 0,
        ...
        return {
            ...record,
            claimCount,
            claimLimitReached: claimCount >= record.claimLimitPerUser
        };
    }

    if (activeOnly) {
        let activeRecords = records.filter((record) => record.isActive && hasStarted(record.startAt, nowMs) && !hasEnded(record.endAt, nowMs));
        activeRecords = await Promise.all(activeRecords.map(decorateCampaign));
        if (hideClaimed && address) {
            activeRecords = activeRecords.filter((record) => !record.claimLimitReached);
        }
    ...
    const decoratedRecords = await Promise.all(records.map(decorateCampaign));
    decoratedRecords.sort((left, right) => {
    ...
    return {
        total: decoratedRecords.length,
        campaigns: decoratedRecords
    };
}
...
export async function claimRewardCampaign(campaignId, address, totalBet = 0) {
    ...
    const vipStatus = buildVipStatus(totalBet);
    const requiredLevel = campaign.minLevel || campaign.minVipLevel;
    if (requiredLevel && !meetsVipLevelRequirement(vipStatus.vipLevel, requiredLevel)) {
        throw new Error(`需達 ${requiredLevel} 才可領取`);
    }
