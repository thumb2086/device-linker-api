...
import { buildVipStatus } from "./vip.js";
...
const SHOP_ITEM_CATALOG = [
    ...
    },
    {
        ...
        type: "buff",
        price: 25000000,
        description: "失敗時返還本金一次，單次保護上限 500 萬。",
        effect: { effectType: "loss_shield", uses: 1, maxProtectedLoss: 5000000, stackable: true, eligibleScopes: ["solo"] }
    },
    {
        ...
        type: "buff",
        price: 90000000,
        description: "共可保護 3 次失敗，每次上限 500 萬。",
        effect: { effectType: "loss_shield", uses: 3, maxProtectedLoss: 5000000, stackable: false, eligibleScopes: ["solo"] }
    },
    {
        ...
        type: "buff",
        price: 800000000,
        description: "15 分鐘內最多保護 3 次，每次上限 1,000 萬。",
        effect: { effectType: "loss_shield", durationMinutes: 15, uses: 3, maxProtectedLoss: 10000000, stackable: false, eligibleScopes: ["solo"] }
    },
    {
        id: "luck_boost",
        name: "幸運增幅卡",
    ...
    { id: "basic_key", name: "普通鑰匙", rarity: "common", type: "key", price: 500000, description: "可開啟稀有與超稀有寶箱。" },
    { id: "advanced_key", name: "高級鑰匙", rarity: "epic", type: "key", price: 8000000, description: "可開啟史詩與神話寶箱。" },
    { id: "master_key", name: "萬用鑰匙", rarity: "legendary", type: "key", price: 80000000, description: "可替代任意一把寶箱鑰匙。" }
];
...
const titleMap = rewardCatalogIndex(TITLE_CATALOG);
...
function isBuffActive(entry, atMs = Date.now()) {
    ...
    if (entry.remainingUses !== undefined && entry.remainingUses <= 0) return false;
    if (!entry.expiresAt) return true;
    return parseDate(entry.expiresAt) > atMs;
}
...
function resolveEquippedTitle(profile, totalBet) {
    const nowMs = Date.now();
...
export function buildRewardSummaryFromProfile(profile, totalBet = 0) {
    const equippedTitle = resolveEquippedTitle(profile, totalBet);
    return {
        ...
        activeBuffs: removeExpiredBuffs(profile.activeBuffs).map((buff) => ({
            ...
            remainingUses: buff.remainingUses || 0,
...
function applyGrantBundleToProfile(profile, bundle, meta = {}) {
...
export async function grantRewardBundle(address, bundle, meta = {}) {
    const profile = await getRewardProfile(address);
    applyGrantBundleToProfile(profile, bundle, meta);
...
export async function activateInventoryItem(address, itemId) {
    ...
    profile.activeBuffs.push({
        ...
        remainingUses: Number(item.effect.uses || 0),
...
export async function openChest(address, chestItemId) {
    const profile = await getRewardProfile(address);
    const chest = shopItemMap.get(chestItemId);
    const rule = CHEST_RULES[chestItemId];
    if (!chest || chest.type !== "chest" || !rule) {
        throw new Error("此寶箱無法開啟");
    }

    consumeInventory(profile, chestItemId, 1);
    const hasMasterKey = asPositiveInt(profile.inventory.master_key || 0, 0) > 0;
    if (rule.keyId && asPositiveInt(profile.inventory[rule.keyId] || 0, 0) <= 0) {
        if (!hasMasterKey) throw new Error("缺少對應鑰匙");
        consumeInventory(profile, "master_key", 1);
    } else if (rule.keyId) {
        consumeInventory(profile, rule.keyId, 1);
    }

    const selectedReward = pickWeightedReward(rule.rewards, getActiveLuckMultiplier(profile));
    applyGrantBundleToProfile(profile, selectedReward.rewards, { source: `chest:${chestItemId}` });
    const saved = await saveRewardProfile(profile);
...
export async function listRewardCampaigns(options = {}) {
    ...
    records.sort((left, right) => {
        const leftActive = left.isActive && parseDate(left.startAt) <= nowMs && parseDate(left.endAt) >= nowMs;
        const rightActive = right.isActive && parseDate(right.startAt) <= nowMs && parseDate(right.endAt) >= nowMs;
...
export async function claimRewardCampaign(campaignId, address, totalBet = 0) {
    ...
    const nowMs = Date.now();
    if (parseDate(campaign.startAt) > nowMs) throw new Error("活動尚未開始");
    if (parseDate(campaign.endAt) < nowMs) throw new Error("活動已結束");

    const vipStatus = buildVipStatus(totalBet);
    if (campaign.minVipLevel && vipStatus.vipLevel !== campaign.minVipLevel) {
