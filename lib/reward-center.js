...
export async function upsertRewardTitle(input) {
    const normalized = normalizeTitleCatalogEntry({
        ...input,
        createdAt: trimText(input && input.createdAt, 64) || nowIso(),
        updatedAt: nowIso()
    });
    if (!normalized.id || !normalized.name) throw new Error("稱號資料不完整");
    if (baseTitleMap.has(normalized.id)) throw new Error("此稱號 ID 已被內建稱號使用");

    const existing = normalizeTitleCatalogEntry(await kv.get(titleCatalogKey(normalized.id)) || {});
...
export async function listRewardCampaigns(options = {}) {
    ...
    const nowMs = Date.now();
    if (activeOnly) {
        const activeRecords = records.filter((record) => record.isActive && hasStarted(record.startAt, nowMs) && !hasEnded(record.endAt, nowMs));
