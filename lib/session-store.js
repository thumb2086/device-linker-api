...
export async function getSession(sessionId) {
    ...
    try {
        ...
        if (hashData && Object.keys(hashData).length > 0) {
            // 自動續期 3600 秒 (1 小時) - 在 Serverless 環境中必須 await，否則可能不會執行
            try {
                await kv.expire(key, 3600);
            } catch (err) {
                console.warn(`Failed to refresh TTL for ${key}:`, err.message);
            }
            return hashData;
        }
    } catch (e) {
    ...
    try {
        ...
        if (legacy) {
            // 傳統 key 也嘗試續期，一樣要 await
            await kv.expire(key, 3600).catch(() => {});
        }
