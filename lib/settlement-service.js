import { ethers } from "ethers";
import { RPC_URL, CONTRACT_ADDRESS, ADMIN_WALLET_ADDRESS } from "./config.js";
import { sendManagedContractTx } from "./admin-chain.js";
import { transferFromTreasuryWithAutoTopup } from "./treasury.js";

/**
 * A centralized service to handle all game settlements.
 * Decouples game logic from on-chain execution.
 */
class SettlementService {
    constructor() {
        this._provider = null;
        this._adminWallet = null;
        this._readContract = null;
        this._writeContract = null;
        this._decimals = null;
        this._lossPoolAddress = null;
    }

    _ensureProvider() {
        if (!this._provider) {
            this._provider = new ethers.JsonRpcProvider(RPC_URL);
        }
    }

    _ensureReadContract() {
        this._ensureProvider();
        if (!this._readContract) {
            this._readContract = new ethers.Contract(CONTRACT_ADDRESS, [
                "function adminTransfer(address from, address to, uint256 amount) public",
                "function decimals() view returns (uint8)",
                "function balanceOf(address) view returns (uint256)"
            ], this._provider);
        }
    }

    _ensureSigner() {
        this._ensureProvider();
        if (!this._adminWallet) {
            let privateKey = String(process.env.ADMIN_PRIVATE_KEY || "").trim();
            if (!privateKey) {
                throw new Error("ADMIN_PRIVATE_KEY is not configured");
            }
            if (!privateKey.startsWith("0x")) privateKey = `0x${privateKey}`;
            this._adminWallet = new ethers.Wallet(privateKey, this._provider);
        }
        if (!this._writeContract) {
            this._writeContract = new ethers.Contract(CONTRACT_ADDRESS, [
                "function adminTransfer(address from, address to, uint256 amount) public",
                "function mint(address to, uint256 amount) public",
                "function decimals() view returns (uint8)",
                "function balanceOf(address) view returns (uint256)"
            ], this._adminWallet);
        }
    }

    _ensureLossPoolAddress() {
        if (this._lossPoolAddress) return;
        const rawAddress = String(process.env.LOSS_POOL_ADDRESS || "").trim();
        if (rawAddress) {
            try {
                this._lossPoolAddress = ethers.getAddress(rawAddress).toLowerCase();
                return;
            } catch (error) {
                throw new Error("LOSS_POOL_ADDRESS format is invalid");
            }
        }
        this._ensureSigner();
        this._lossPoolAddress = String(this._adminWallet.address || "").toLowerCase();
    }

    get provider() {
        this._ensureProvider();
        return this._provider;
    }

    get contract() {
        this._ensureReadContract();
        return this._readContract;
    }

    get writeContract() {
        this._ensureSigner();
        return this._writeContract;
    }

    get lossPoolAddress() {
        this._ensureLossPoolAddress();
        return this._lossPoolAddress;
    }

    async getDecimals() {
        this._ensureReadContract();
        if (this._decimals === null) {
            try {
                const rawDecimals = await this._readContract.decimals();
                const numericDecimals = Number(rawDecimals);
                if (!Number.isFinite(numericDecimals) || numericDecimals < 0) {
                    throw new Error("Invalid decimals");
                }
                this._decimals = numericDecimals;
            } catch (error) {
                console.error("Failed to fetch decimals, defaulting to 18", error);
                this._decimals = 18;
            }
        }
        return this._decimals;
    }

    /**
     * Executes a complete settlement: deducts bet and pays out winnings.
     */
    async settle({ userAddress, betWei, payoutWei, source, meta = {}, waitForReceipt = true }) {
        this._ensureSigner();
        this._ensureLossPoolAddress();
        const results = {
            betTxHash: null,
            payoutTxHash: null,
            betTxContext: null,
            payoutTxContext: null,
            betTransferred: false,
            success: false,
            error: null
        };

        try {
            const contract = this._writeContract;
            // 1. Deduct Bet (User -> Loss Pool)
            if (betWei > 0n) {
                try {
                    const betTx = await sendManagedContractTx(
                        contract,
                        "adminTransfer",
                        [userAddress, this._lossPoolAddress, betWei],
                        { txSource: source, txMeta: { ...meta, stage: "bet" }, waitForReceipt }
                    );
                    results.betTxHash = betTx.hash;
                    results.betTxContext = betTx.managedContext || null;
                    results.betTransferred = waitForReceipt;
                } catch (betError) {
                    throw new Error(`扣除下注失敗 (${source}): ${betError.message}`);
                }
            } else {
                results.betTransferred = true;
            }

            // 2. Pay Out (Loss Pool/Treasury -> User)
            if (payoutWei > 0n) {
                try {
                    const payoutTx = await transferFromTreasuryWithAutoTopup(
                        contract,
                        this._lossPoolAddress,
                        userAddress,
                        payoutWei,
                        { txSource: source, txMeta: { ...meta, stage: "payout" }, waitForReceipt }
                    );
                    results.payoutTxHash = payoutTx.hash;
                    results.payoutTxContext = payoutTx.managedContext || null;
                } catch (payoutError) {
                    throw new Error(`發放獎勵失敗 (${source}): ${payoutError.message}`);
                }
            }

            results.success = true;
            return results;

        } catch (error) {
            results.error = error.message;
            const wrappedError = new Error(error.message);
            wrappedError.results = results;
            throw wrappedError;
        }
    }
}

// Single instance, but initialization is deferred until first call.
export const settlementService = new SettlementService();
