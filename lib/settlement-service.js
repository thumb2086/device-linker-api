...
export class SettlementService {
     ...
     * @param {bigint} params.betWei - The amount to deduct (set to 0 if already deducted).
     ...
     * @returns {Promise<Object>} The transaction result hashes and status flags.
     */
    async settle({ userAddress, betWei, payoutWei, source, meta = {} }) {
        ...
        try {
            ...
            if (betWei > 0n) {
                ...
                // If betWei is 0, we assume it was already transferred in a previous attempt
                results.betTransferred = true;
            }
            ...
            // Throw the results object so the caller knows the progress (e.g., if bet was transferred)
