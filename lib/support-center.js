...
async function scanRecords(prefix, normalizer) {
    const keys = [];
...
export async function listIssueReports(options = {}) {
    ...
    const records = await scanRecords(ISSUE_REPORT_PREFIX, normalizeIssueRecord);
...
export async function listAnnouncements(options = {}) {
    ...
    const records = await scanRecords(ANNOUNCEMENT_PREFIX, normalizeAnnouncementRecord);
