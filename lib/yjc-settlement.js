...
class YjcSettlementService {
    ...
    async burnFrom(address, amount, context = {}) {
        const decimals = await this.getDecimals();
        const normalized = Number.isFinite(Number(amount)) ? Math.max(0, Math.floor(Number(amount))) : 0;
        if (normalized <= 0) throw new Error("Burn amount must be greater than 0");
        const weiAmount = ethers.parseUnits(String(normalized), decimals);
        const balance = await this.contract.balanceOf(address);
        if (balance < weiAmount) {
            throw new Error("Target address YJC balance is insufficient for burn");
        }
        return sendManagedContractTx(
            this.writeContract,
            "burn",
            [address, weiAmount],
            { txSource: context.source || "admin_burn_yjc", txMeta: context.meta || {} }
        );
    }

    async getTotalSupplyFormatted() {
        try {
