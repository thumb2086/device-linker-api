...
const YJC_VIP_CACHE_TTL_SECONDS = 7 * 24 * 60 * 60;
...
function yjcVipCacheKey(address) {
    return `yjc_vip_status:${normalizeAddressKey(address)}`;
}
...
async function readCachedYjcVipStatus(address) {
    try {
        const cached = await kv.get(yjcVipCacheKey(address));
        if (!cached || typeof cached !== "object") return null;
        const balance = Math.max(0, Math.floor(toSafeNumber(cached.balance, 0)));
        return {
            available: true,
            stale: true,
            cachedAt: cached.cachedAt || null,
            ...buildYjcVipStatusFromBalance(balance, "cache_fallback")
        };
    } catch (error) {
        console.error("YJC VIP cache read failed:", error?.message || error);
        return null;
    }
}

async function writeCachedYjcVipStatus(address, balance) {
    try {
        await kv.set(
            yjcVipCacheKey(address),
            {
                balance: Math.max(0, Math.floor(toSafeNumber(balance, 0))),
                cachedAt: new Date().toISOString()
            },
            { ex: YJC_VIP_CACHE_TTL_SECONDS }
        );
    } catch (error) {
        console.error("YJC VIP cache write failed:", error?.message || error);
    }
}
...
export async function resolveYjcVipStatus(address) {
    ...
    try {
        ...
        await writeCachedYjcVipStatus(normalizedAddress, normalized);
        return { available: true, ...buildYjcVipStatusFromBalance(normalized, "onchain") };
    } catch (error) {
        const cached = await readCachedYjcVipStatus(normalizedAddress);
        if (cached) {
            return {
                ...cached,
