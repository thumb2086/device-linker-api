...
export class KVUserRepository implements IUserRepository {
  ...
  async getUserById(id: string) { return await kv.get<any>(`pg_mock:user:${id}`); }
...
export class KVSessionRepository implements ISessionRepository {
  async saveSession(session: any) { await kv.set(`pg_mock:session:${session.id}`, session, { ex: 86400 * 7 }); }
  async getSessionById(id: string) { return await kv.get<any>(`pg_mock:session:${id}`); }
}
...
export class KVMarketRepository implements IMarketRepository {
  async getAccount(address: string) { return await kv.get<any>(`market_account:${address.toLowerCase()}`); }
  async saveAccount(address: string, userId: string, account: any) { await kv.set(`market_account:${address.toLowerCase()}`, account); }
...
export class KVMetaRepository implements IMetaRepository {
  async saveRewardGrant(grant: any) { await kv.lpush('pg_mock:reward_grants', grant); }
  async saveMarketOrder(order: any) { await kv.lpush('pg_mock:market_orders', order); }
}
